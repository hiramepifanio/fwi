...
int main() {
    ...
    float** grad2 = grad_recon3(wavelet, shot, vel0, dt, dz, dx, nt, nz, nx, border, souz, soux, recz, recx, nrecs);
...
float** velocityMap(int nz, int nx) {
    int vel_shape[] = {nz, nx};
    float** velocityMap = alloc_float2d(3000, vel_shape);

    for(int i = 0; i < nz; i++) 
        for(int j = 0; j < nx; j++) 
            velocityMap[i][j] += i*3000.0/nz;
