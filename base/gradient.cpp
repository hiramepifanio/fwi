...
int main() {
    ...
    for(int t = 0; t < nt; t++) {
        ...
        for(int i = 0; i < nx; i++) {
            wave2[border + recz][border + i] += vel0[souz + border][soux + border]*vel0[souz + border][soux + border]*dt*dt*diff[nt - t - 1][i];
        }
        ...
        if ((nt - 1 - t)%reamos == 0)
            for(int i = 0; i < nz; i++)
                for(int j = 0; j < nx; j++)
                    d2u[i][j][t/reamos] *= wave2[i + border][j + border];
    ...
    int shape2[] = {nz, nx};
    saveShot(grad, shape2, "data/gradient", "grad");
